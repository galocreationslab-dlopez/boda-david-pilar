...
import type { SeparadorDiseno, TipoSeccionDiseno } from "@/config/wedding.config";
...
export default async function PaginaPrincipal() {
  ...
  const fallbackSections: Array<{ id: string; tipo: TipoSeccionDiseno; titulo: string }> = [
  ...
  const orderedSections = visibleSections.length > 0
    ? visibleSections.map((s) => ({ id: s.id, tipo: s.tipo, titulo: s.titulo || s.nombre }))
    : fallbackSections;
